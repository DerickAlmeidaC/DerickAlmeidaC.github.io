const IMC = () => {
  ...
  if ((altura > 0) & (peso > 0)) {
    ...
    resultado < 18.5
      ...
      : resultado <= 34.9
      ? (classificação = "com Obesidade classe I")
      : resultado <= 39.9
      ? (classificação = "com Obesidade classe II")
      : resultado >= 40
      ? (classificação = "com Obesidade classe III")
      : (classificação = "outro");

    result_IMC.innerHTML = `${nome} está com classificação ${classificação} \n, Porque : "${peso} /(${altura} * ${altura} ) = ${resultado.toFixed(
      2
    )}" que é ${classificação}`;
  } else {
